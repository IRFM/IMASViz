...
class VizProfiles_plugin(VizPlugin):
    ...
    def __init__(self):
        ...
        self.IDS_parameters = {}
        self.supported_idss = ['edge_profiles', 'core_profiles', 'equilibrium', 'core_sources']
        ...
        try:
            ...
            if selected_ids == 'core_profiles':
                ...
                list_of_filters = ['profiles_1d(0)/grid', 'profiles_1d(0)/electrons', 'profiles_1d(0)/ion',
                                   ...
                                   'profiles_1d(0)/conductivity_parallel', 'profiles_1d(0)/e_field',
                                   'profiles_1d(0)/rotation', 'profiles_1d(0)/q', 'profiles_1d(0)/magnetic_shear',
                                   'profiles_1d(0)/phi_potential', 'global_quantities']
                tab_names = ['profiles_1d/grid', 'profiles_1d/electrons', 'profiles_1d/ion', 'profiles_1d/neutral',
                             'profiles_1d/t_i_average', 'profiles_1d/n_i', 'profiles_1d/momentum_tor',
                             'profiles_1d/zeff', 'profiles_1d/pressure', 'profiles_1d/j',
    ...
    def getAllEntries(self):
        ...
        if selected_ids in self.supported_idss:
            if selected_ids == 'core_sources':
                return [(0, '0D/1D visualization of source(i1)/global_quantities(itime) along coordinate1 axis...'),
                        (1, '0D/1D visualization of source(i1)/global_quantities(itime) along time axis...'),
                        (2, '0D/1D visualization of source(i1)/profiles_1d(itime) along coordinate1 axis...'),
                        (3, '0D/1D visualization of source(i1)/profiles_1d(itime) along time axis...')]
            else:
                return [(0, '0D/1D visualization along coordinate1 axis...'),
                        (1, '0D/1D visualization along time axis...')]
        else:
            return []
    ...
    def getDescription(self):
        ...
        return "Make 0D/1D IDS plots from data arrays embedded in dynamic array of structures. \n" \
