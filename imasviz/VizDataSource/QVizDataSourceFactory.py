    ...
    def create(self, shotNumber, runNumber=0, userName=None,
        ...
        if machineName == 'TCV' or machineName == 'AUG':
            ...
            if not os.path.exists(exp2imasFile):
                raise ValueError("Access to " + machineName +
                                 " requires a file named '.exp2imas'  located "
                                 "in your home directory containing one line:"
        ...
        if machineName == 'WEST':
            ...
            if not os.path.exists(west_tunnel_file):
                raise ValueError("Access to " + machineName +
                                 " requires a file named '.west_tunnel' "
                                 "located in your home directory containing "
                                 "one line:" + machineName +
                                 " username password")
        ...
        if dataSourceName == QVizGlobalValues.IMAS_NATIVE:
            dataSource = QVizIMASDataSource(dataSourceName, userName,
                                            imasDbName, shotNumber, runNumber)
            return dataSource
            ...
            if machineName is None or machineName == '':
                ...
                dataSource = QVizIMASPublicDataSource(dataSourceName,
                                                      machineName,
                                                      shotNumber, runNumber)
                return dataSource
            else:
